...
namesIMGE = np.array(['baboon.png','butterfly.png', 'f16.png', 'leaves.png', 'lena.png', 'parrots.png', 'peppers.png','starfish.png' ])
...
#for k in range(1,25):#25
	...
	if denb[-1] == 'U':
		...
		name = nameimg.replace('.png','_uniformblur.png')
	else:
		name = nameimg.replace('.png','_blurred.png')
	print(name)
	
	fname = ruta + 'degraded_' + name
	...
	for t in range(5):
		...
		for lam in LAMval:
			...
			full_net =  RiemannianGradientNew(input_depth, dtype, pad, height=img_pil.size[1], width=img_pil.size[0], upsample_mode='bilinear' ).type(dtype)
			...
			fname = ruta + str(Lambda) + 't'+ str(t) + nameimg 
			...
			fname = './restoration/P'+ Nprueb +'/BestPnsr'+ denb +'_P'+ Nprueb +'_' + str(Lambda) + 't'+ str(t) + nameimg
		...
		fname = ruta + 'P'+ Nprueb + denb +'PNSR' + 't'+ str(t) + name
		...
		fname = ruta  + 'P'+ Nprueb + denb +'MAXval' + 't'+ str(t) + name
