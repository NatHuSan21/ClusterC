...
for k in range(1,25):#for k in listaRep:# 25 # 
    ...
    if denb[-1] == 'U':
        ...
        name = nameimg.replace('.png','_blurred.png')#'_blurred_uniform.png')
    ...
    img_degraded_np2[0,:,:]=(1./(sqrt(3)))*(np.sum(img_degraded_np,axis=0))
    ...
    for lam in LAMval:
        ...
        full_net =  RiemannianGradientNew(input_depth, dtype, pad, height=img_height, width= img_width, upsample_mode='bilinear').type(dtype)
        ...
        def closure():
            ...
            opp1=(1./(sqrt(3)))*torch.unsqueeze(torch.sum(net_output,dim=1),dim=1)
