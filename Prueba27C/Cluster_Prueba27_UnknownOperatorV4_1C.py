...
for k in listCaid:#25
    ...
    for lam in LAMval:
        ...
        ruta = './restoration/P271C/denoised_P271C_'
        fname = ruta + str(Lambda)  + nameimg 
        out_img_avg_pil.save(fname)

        #Save best image 
        imgPNSRmax_pil = np_to_pil(imgPNSRmax_np)
        fname = './restoration/P271C/BestPnsr_P271C_' + str(Lambda) + nameimg 
    ...
    ruta = './txt271C/'
    name = nameimg.replace('.png','.csv')
    fname = ruta  + 'P271CdenMAXval' + name
